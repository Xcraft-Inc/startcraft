...
const postinstall = watt(function*(next) {
  ...
  try {
    ...
    /* Main dependencies */
    if (pkgs.length) {
      const list = _.chunk(pkgs, 64);
      for (const slist of list) {
        const subjects = config.npmInstallArgs.concat(slist);
    ...
    for (const sub in config.substitutions) {
      ...
      if (!fse.existsSync(substitute)) {
        clc.yellowBright(
          `-> skip substitution of ${sub} by ${
            config.substitutions[sub]
          } because ${config.substitutions[sub]} module is not installed`
        );
        continue;
      }
