...
const postinstall = watt (function * (next) {
  ...
  const pkgs = Object.keys (list).map (mod => {
    /* It's necessary because [^] is stripped on Window with the spawn
     * of npm by cmd. Event with escaping like ^^ it's not working. Then
     * the solution is to use normalize ranges.
     */
