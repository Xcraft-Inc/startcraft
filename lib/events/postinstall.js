...
const postinstall = watt (function * (next) {
  ...
  const pkgInfos = config.modules
    .map (pkgPath => {
      const pkgJsonPath = path.join (root, pkgPath, 'package.json');
      return {
        def: JSON.parse (fs.readFileSync (pkgJsonPath)),
        path: path.join (root, pkgPath)
      };
    })
    .filter (pkgDef => helpers.isOsCompatible (pkgDef, process.platform))
    .forEach (pkgDef => {
  ...
  pkgInfos.forEach (info => {
    symlink (
      info.path,
      path.join (root, 'node_modules', path.basename (info.path))
    );
  });
