...
const npm = function (verb, subjects, cwd, next) {
  ...
  console.log (
    clc.greenBright (
      `${agent} ${verb} ${subjects.map (s => `"${s}"`).join (' ')}`
    )
  );
...
function parsePackage (pkgDef) {
  ...
  [pkgDef.dependencies, pkgDef.devDependencies]
    ...
    .forEach (deps => {
      Object.keys (deps)
        .filter (
          pkg => packages.indexOf (pkg) === -1 && config.exclude.indexOf (pkg)
        )
        .forEach (pkg => {
          if (!list[pkg]) {
            list[pkg] = [];
...
const postinstall = watt (function* (next) {
  ...
  console.error (
    clc.yellowBright (
      `Please, remove the lock files (package-lock.json and/or yarn.lock) because it conflicts with startcraft`
    )
  );
});
