...
const postinstall = watt(function*(next) {
  ...
  try {
    ...
    for (const sub in config.substitutions) {
      ...
      const dst = path.join(root, 'node_modules', sub);

      if (!fse.existsSync(src)) {
        clc.yellowBright(
          `-> skip substitution of ${sub} because the module is not installed`
        );
        continue;
      }

      fse.removeSync(src);
      fse.symlinkSync(config.substitutions[sub], dst, 'junction');
    }
  } finally {
